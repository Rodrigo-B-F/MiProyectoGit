...
# --- Mapeo estándar de columnas de inventario ---
# Define las columnas que se mostrarán en las vistas de inventario con sus nombres en español
STANDARD_INVENTORY_COLUMNS = {
    "name": "Nombre",
...
class ViewScreen(Screen):
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "exit_view_menu":
            ...
            self.app.push_screen(InventoryViewerScreen(title="PRODUCTOS ACTIVOS", list_function=list_products_inventory, list_args=[1], header_map=STANDARD_INVENTORY_COLUMNS))
        elif event.button.id == "view_inactive":
            self.app.push_screen(InventoryViewerScreen(title="PRODUCTOS INACTIVOS", list_function=list_products_inventory, list_args=[2], header_map=STANDARD_INVENTORY_COLUMNS))
        elif event.button.id == "view_available":
            self.app.push_screen(InventoryViewerScreen(title="PRODUCTOS CON STOCK", list_function=list_available_products, header_map=STANDARD_INVENTORY_COLUMNS))
        elif event.button.id == "view_out_of_stock":
            self.app.push_screen(InventoryViewerScreen(title="PRODUCTOS SIN STOCK", list_function=list_out_of_stock_products, header_map=STANDARD_INVENTORY_COLUMNS))
...
class SearchProductScreen(Screen):
    ...
    def execute_search(self) -> None:
        ...
        try:
            ...
            headers = STANDARD_INVENTORY_COLUMNS.values()
            table.add_columns(*headers)
            original_keys = STANDARD_INVENTORY_COLUMNS.keys()
