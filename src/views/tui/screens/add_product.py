...
class AddProductFormScreen(Screen):
    ...
    def handle_save(self) -> None:
        ...
        try:
            ...
            if not all([data["name"], data["barcode"], data["category_name"], data["sale_price"], data["initial_quantity"]]):
            ...
            try:
                ...
                feedback.update(" [b]ERROR:[/b] Precio y Cantidad deben ser números válidos.")
                return

            success, message = add_product(
                data["name"], data["barcode"], data["category_name"], data["location"],
                sale_price_f, initial_quantity_i
            )
