...
class Product(BaseModel):
    ...
    image_url = TextField(null=True)               # opcional, si quieres guardar ruta/URL de imagen
...
def record_sale(items):
    """
    Registra una venta y actualiza inventario y movimientos.
    ...
    with db.atomic():
        ...
        for it in items:
            ...
            inv, _ = Inventory.get_or_create(product=product)
            inv.quantity = inv.quantity - qty
            inv.last_updated = datetime.datetime.now()
            inv.save()

            # Registrar movimiento
            StockMovement.create(
                product=product, change=-qty,
                reason='sale', reference=str(sale.id)
            )
